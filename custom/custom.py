        ...
        for url in url_list:
            ...
            for ul in ul_list:
                try:
                    ...
                    port = port_sum >> 3
                    res.append('{}:{}'.format(ip, port).replace(' ' , ''))
        ...
        for each_url in url_list:
            for i in range(1, page_count + 1):
                ...
                for proxy in proxy_list:
                    try:
                        res.append(':'.join(proxy.xpath('./td/text()')[0:2]).replace(' ' , ''))
        ...
        for each_proxy in proxy_list:
            try:
                ...
                for _ in each_proxy.xpath(".//span[contains(@class, 'port')]"
                    ...
                    port += (ord(_) - ord('A'))
                port /= 8
                res.append('{}:{}'.format(ip_addr, int(port)).replace(' ' , ''))
    ...
    def freeProxy05():
        ...
        for url in url_list:
            ...
            for tr in proxy_list[1:]:
                res.append(':'.join(tr.xpath('./td/text()')[0:2]).replace(' ' , ''))
    ...
    def freeProxy09(page_count=1):
        ...
        for i in range(1, page_count + 1):
            ...
            for index, tr in enumerate(html_tree.xpath("//table//tr")):
                if index == 0:
                    continue
                res.append(":".join(tr.xpath("./td/text()")[0:2]).strip().replace(' ' , ''))
...
my_crawlers = [GetFreeProxy.freeProxy01,
                GetFreeProxy.freeProxy03,
                GetFreeProxy.freeProxy04,
                GetFreeProxy.freeProxy05,
                GetFreeProxy.freeProxy07,
                GetFreeProxy.freeProxy08,
                GetFreeProxy.freeProxy09,
                GetFreeProxy.freeProxy13,
                GetFreeProxy.freeProxy14]